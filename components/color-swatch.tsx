import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/library/tooltip";
...
export function ColorSwatch({ color, globalFormat }: ColorSwatchProps) {
    ...
    const isDark = hex ? Number.parseInt(hex.replace("#", ""), 16) < 0xffffff / 1.8 : false;
                    ...
                    <TooltipProvider>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                ...
                                >
                                    {copied ? <Check strokeWidth={2.3} className="h-4 w-4 text-green-500" /> : <Copy strokeWidth={2.3} className="h-4 w-4" />}
                                </button>
                            </TooltipTrigger>
                            <TooltipContent>
