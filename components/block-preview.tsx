...
const CopyButton = ({ content }: { content: string }) => {
  ...
  return (
    <button
      className="flex items-center bg-muted shadow-sm py-3 md:py-2.5 px-3 rounded-[7px] space-x-1"
    ...
    >
      ...
      <span className="font-medium text-xs text-nowrap hidden sm:block">
        Copy Code
      </span>
    </button>
  )
  ...
  useEffect(() => {
    const updatePreviewWidth = () => {
      ...
      if (active === "desktop") {
        ...
        setPreviewWidth(
          windowWidth < targetWidth ? "100%" : screensWidth[active]
        )
      }
    }
