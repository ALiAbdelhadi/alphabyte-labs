...
export async function GET(request: NextRequest) {
  try {
    ...
    if (baseComponent) {
      ...
      filePath = path.join(process.cwd(), 'registry-components', 'examples', folderPath, `${componentName}.tsx`);

      try {
        await fs.access(filePath);
        fileContent = await fs.readFile(filePath, 'utf-8');
      } catch (error) {
        filePath = path.join(process.cwd(), 'registry-components', 'examples', `${componentName}.tsx`);
        fileContent = await fs.readFile(filePath, 'utf-8');
      }
    } else {
      filePath = path.join(process.cwd(), 'registry-components', 'examples', `${componentName}.tsx`);
