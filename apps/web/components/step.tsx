    ...
    <div className={cn("flex flex-col w-full relative", className)}>
      {Children.map(children, (child, index) => (
        <div className="relative pl-10">
          {index < length - 1 && (
            <span className="absolute left-4 top-8 w-px h-full bg-zinc-200 dark:bg-zinc-700" />
          )}
          <span className="absolute left-0 top-1 w-8 h-8 flex items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 text-sm font-medium">
            {index + 1}
          </span>
