...
export default function DocsSidebarItem(props: DocsSidebarItemProps) {
    ...
    const itemKey = href?.replace(/^\//, "") || ""

    const content = (
        <div className={cn("space-x-3 rtl:space-x-reverse flex rtl:flex-row-reverse",)} >
            <span className="text-[1rem] md:text-sm text-foreground/75">{t(itemKey)}</span>
            {isNewComponent && (
