...
export default function ProductsList() {
    return (
        <>
            <div className="flex items-center justify-between mb-8 max-w-[150px] ">
            ...
            <div className="flex h-screen border border-border p-2 rounded-lg">
                <Sidebar />
                <main className="flex-1 bg-[#f5f0ea] lg:p-8 p-4 overflow-y-auto ">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
                        {products.map((product) => (
                            <Link
                                href={`/products/${product.id}`}
                                key={product.id}
                                className="group bg-[#f5f0ea] rounded-md overflow-hidden transition-all duration-300 hover:opacity-90">
