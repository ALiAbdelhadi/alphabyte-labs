...
export function rgbToHex(rgb: RgbColor): string {
    try {
        const color = { mode: "rgb", r: rgb.red / 255, g: rgb.green / 255, b: rgb.blue / 255 }
...
export function generatePalette(baseHex: string, steps = 10): string[] {
    ...
    try {
        ...
        for (let i = 1; i <= steps; i++) {
            const lightness = Math.min(baseHsl.lightness + (i * (100 - baseHsl.lightness)) / steps, 98)
            const saturation = Math.max(baseHsl.s - (i * baseHsl.s) / (steps * 2), baseHsl.s / 2)
            palette.push(
                hslToHex({
                    h: baseHsl.h,
                    s: saturation,
        ...
        // توليد ظلال أغمق
        for (let i = 1; i <= steps; i++) {
            const lightness = Math.max(baseHsl.lightness - (i * baseHsl.lightness) / steps, 5)
            const saturation = Math.min(baseHsl.s + (i * (100 - baseHsl.s)) / (steps * 2), baseHsl.s * 1.2)
...
export function generateTheme(primaryHex: string): Record<string, string> {
    ...
    try {
        ...
        const theme = {
            ...
            secondary: hslToCssVar({ h: primary.h, s: Math.max(primary.s - 30, 10), lightness: 65 }),
            "secondary-foreground": hslToCssVar({ h: primary.h, s: 10, lightness: 20 }),

            // لون التأكيد (لون متشابه)
            accent: hslToCssVar({ h: analogous1Hue, s: primary.s, lightness: 45 }),
            "accent-foreground": hslToCssVar({ h: analogous1Hue, s: 5, lightness: 98 }),
...
export function generateDarkTheme(lightTheme: Record<string, string>): Record<string, string> {
